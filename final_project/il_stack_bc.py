...
class ManiSkill2Dataset(Dataset):
    def __init__(self, dataset_file: str, load_count=-1,extrainfo = False) -> None:
        ...
        for eps_id in tqdm(range(load_count)):
            ...
            obs = convert_observation(trajectory["obs"],extrainfo=extrainfo)
...
class MixCNN(nn.Module):
    ...
    # also using qpos state info, but should doesn't matter
...
class VisPolicy(nn.Module):
    def __init__(
        ...
        #self.feature_extractor = MixCNN(image_size, in_channels,state_size=state_size)
        mlp_layers = []
...
if __name__ == 'main':
    ...
    th.manual_seed(0)
    policy = VisPolicy(image_size=rgbd_shape[1:], in_channels=rgbd_shape[0],state_size=obs['state'].shape[0],
    ...
    while steps < iterations:
        ...
        for batch in dataloader:
            steps += 1
            obs, actions = batch
            #print(obs['rgbd'].shape)            #[100,8,128,128]
            #print(obs['state'].shape)           #[100,23]
            #print(actions.shape)                #[100,7]
            #break
    ...
    while i < num_episodes:
        ...
        obs = convert_observation(obs)  #,extrainfo=False
        obs_device = dict()
